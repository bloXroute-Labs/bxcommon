...
class BTCObjectHashTests(unittest.TestCase):

    def setUp(self):
        self.int_hash_31a = BTCObjectHash(bytearray([i for i in range(SHA256_HASH_LEN)]))
        self.int_hash_31b = BTCObjectHash(memoryview(bytearray([i for i in range(SHA256_HASH_LEN)])))
        self.int_hash_32 = BTCObjectHash(bytearray([i for i in range(1, SHA256_HASH_LEN + 1)]))
        self.int_hash_all_0 = BTCObjectHash(bytearray([0] * SHA256_HASH_LEN))

    def test_init(self):
        self.tmp = BTCObjectHash()
        self.assertEqual(tmp.binary, '')
