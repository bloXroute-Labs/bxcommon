...
class ConnectionPoolTest(unittest.TestCase):
    ...
    def test_init(self):
        self.assertEqual([None] * ConnectionPool.INITIAL_FILENO, self.conn_pool1.byfileno)
        self.assertEqual(ConnectionPool.INITIAL_FILENO, self.conn_pool1.len_fileno)
        self.assertEqual({}, self.conn_pool1.byipport)
        self.assertEqual(defaultdict(lambda: 0), self.conn_pool1.count_conn_by_ip)
        self.assertEqual(0, self.conn_pool1.num_peer_conn)
    ...
    def test_get_byipport(self):
        self.add_conn()
        self.assertTrue(self.conn1, self.conn_pool1.get_byipport(self.ip1, self.port1))
        with self.assertRaises(KeyError):
            self.conn_pool1.get_byipport(self.ip1, 1)

    def test_get_by_fileno(self):
        self.add_conn()
        self.assertEqual(self.conn_pool1.get_byfileno(self.fileno1), self.conn1)
        self.assertEqual(self.conn_pool1.get_byfileno(self.fileno2), self.conn2)
    ...
    def test_get_num_conn_by_ip(self):
        self.add_conn()
        self.assertEqual(self.conn_pool1.get_num_conn_by_ip(self.ip1), 1)
        self.assertEqual(self.conn_pool1.get_num_conn_by_ip(self.ip2), 2)
        self.assertEqual(self.conn_pool1.get_num_conn_by_ip("222.222.222.222"), 0)
    ...
    def test_delete(self):
        ...
        with self.assertRaises(KeyError):
            self.assertEqual(self.conn_pool1.get_byipport(self.ip1, self.port1), None)

        self.conn_pool1.delete(self.conn2)
        self.assertIsNone(self.conn_pool1.get_byfileno(self.fileno2))
        self.assertEqual(self.conn_pool1.count_conn_by_ip[self.ip2], 1)
    ...
    def test_iter(self):
        ...
        self.assertEqual(pool_iter.next(), self.conn1)
        self.assertEqual(pool_iter.next(), self.conn2)
        self.assertEqual(pool_iter.next(), self.conn3)
        with self.assertRaises(StopIteration):
            pool_iter.next()

    def test_len(self):
        self.assertEqual(len(self.conn_pool1), 0)
        self.add_conn()
        self.assertEqual(len(self.conn_pool1), 3)
        self.conn_pool1.delete(self.conn2)
        self.assertEqual(len(self.conn_pool1), 2)
