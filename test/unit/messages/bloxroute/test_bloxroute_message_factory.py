...
class BloxrouteMessageFactory(AbstractTestCase):
    ...
    def test_message_preview_success_all_types(self):
        self.get_message_preview_successfully(HelloMessage(1), HelloMessage.MESSAGE_TYPE, UL_INT_SIZE_IN_BYTES)
        ...
        self.get_message_preview_successfully(BroadcastMessage(self.HASH, blob), BroadcastMessage.MESSAGE_TYPE,
                                              SHA256_HASH_LEN + len(blob))
        self.get_message_preview_successfully(TxMessage(self.HASH, blob, sid=12), TxMessage.MESSAGE_TYPE,
                                              SHA256_HASH_LEN + UL_INT_SIZE_IN_BYTES + len(blob))
        self.get_message_preview_successfully(KeyMessage(self.HASH, bytearray(1 for _ in range(KEY_SIZE))),
                                              KeyMessage.MESSAGE_TYPE, SHA256_HASH_LEN + KEY_SIZE)
    ...
    def test_message_hash_preview(self):
        blob = bytearray(1 for _ in xrange(4))
        self.get_message_hash_preview_successfully(BroadcastMessage(self.HASH, blob), self.HASH)
        self.get_message_hash_preview_successfully(TxMessage(self.HASH, blob, sid=12), self.HASH)
        self.get_message_hash_preview_successfully(KeyMessage(self.HASH, bytearray(1 for _ in range(KEY_SIZE))),
                                                   self.HASH)

    def test_message_hash_preview_incomplete(self):
        blob = bytearray(1 for _ in xrange(4))
        broadcast_message = BroadcastMessage(self.HASH, blob)
        ...
        self.assertEquals(SHA256_HASH_LEN + len(blob), payload_length)
    ...
    def test_create_message_success_all_types(self):
        hello_message = self.create_message_successfully(HelloMessage(1), HelloMessage)
        ...
        blob = bytes(4)
        broadcast_message = self.create_message_successfully(BroadcastMessage(self.HASH, blob), BroadcastMessage)
        ...
        tx_message = self.create_message_successfully(TxMessage(self.HASH, tx_val, sid=sid), TxMessage)
        ...
        key_message = self.create_message_successfully(
            KeyMessage(self.HASH, bytearray(1 for _ in range(KEY_SIZE))), KeyMessage)
