...
class TransactionTest(AbstractTestCase):
    def test_legacy_transaction_from_bytes(self):
        ...
        self.assertEqual(
            Sha256Hash.from_string(eth_fixtures.LEGACY_TRANSACTION_HASH), transaction.hash(),
        )
        self.assertEqual("0xc296825bf94ca41b881390955c2731c1d3eaa059", transaction.from_address())
        self.assertEqual(37, transaction.v)

    def test_legacy_transaction_to_json(self):
        transaction_json = rlp.decode(eth_fixtures.LEGACY_TRANSACTION, Transaction).to_json()
        self.assertEqual(f"0x{eth_fixtures.LEGACY_TRANSACTION_HASH}", transaction_json["hash"])
        self.assertEqual("0x0", transaction_json["type"])
        self.assertEqual("0xc296825bf94ca41b881390955c2731c1d3eaa059", transaction_json["from"])
    ...
    def test_legacy_transaction_eip_2718_from_bytes(self):
        ...
        re_encoded = rlp.encode(transaction)
        self.assertEqual(Transaction.deserialize(eth_fixtures.LEGACY_TRANSACTION), transaction)
        self.assertEqual(transaction, rlp.decode(re_encoded, Transaction))

    def test_legacy_transaction_from_json(self):
    ...
    def test_eip_155_transaction_from_bytes_to_json(self):
        ...
        self.assertEqual(
            f"0x{eth_fixtures.TRANSACTION_EIP_155_HASH}", parsed_transaction_json["hash"],
        )
        self.assertEqual(
    ...
    def test_pre_eip_155_transaction_from_bytes_to_json(self):
        transaction = rlp.decode(eth_fixtures.TRANSACTION_PRE_EIP_155, Transaction)
        parsed_transaction_json = transaction.to_json()
        ...
        self.assertEqual(
            f"0x{eth_fixtures.TRANSACTION_PRE_EIP_155_HASH}", parsed_transaction_json["hash"],
        )
        self.assertEqual(
    ...
    def test_access_list_transaction_from_bytes(self):
        transaction: Transaction = rlp.decode(eth_fixtures.ACL_TRANSACTION, Transaction)
        re_encoded = rlp.encode(transaction)
        self.assertEqual(transaction, rlp.decode(re_encoded, Transaction))

        self.assertEqual(EthTransactionType.ACCESS_LIST, transaction.transaction_type)
        ...
        self.assertEqual("0x0087c5900b9bbc051b5f6299f5bce92383273b28", transaction.from_address())
        self.assertEqual(3, len(transaction.access_list))

    def test_access_list_transaction_to_json(self):
        transaction_json = rlp.decode(eth_fixtures.ACL_TRANSACTION, Transaction).to_json()
        self.assertEqual(f"0x{eth_fixtures.ACL_TRANSACTION_HASH}", transaction_json["hash"])
        self.assertEqual("0x1", transaction_json["type"])
        self.assertEqual("0x0087c5900b9bbc051b5f6299f5bce92383273b28", transaction_json["from"])
    ...
    def test_access_list_transaction_from_json(self):
        ...
        for i, access in enumerate(eth_fixtures.ACL_TRANSACTION_JSON["accessList"]):
    ...
    def test_dynamic_fee_transaction_from_bytes(self):
        transaction: Transaction = rlp.decode(eth_fixtures.DYNAMIC_FEE_TRANSACTION, Transaction)
        re_encoded = rlp.encode(transaction)

        self.assertEqual(transaction, rlp.decode(re_encoded, Transaction))
        self.assertEqual(EthTransactionType.DYNAMIC_FEE, transaction.transaction_type)
        self.assertIsInstance(transaction, DynamicFeeTransaction)
        self.assertEqual(
            Sha256Hash.from_string(eth_fixtures.DYNAMIC_FEE_TRANSACTION_HASH), transaction.hash(),
        )
        self.assertEqual(eth_fixtures.DYNAMIC_FEE_FROM_ADDRESS, transaction.from_address())

        self.assertEqual(0, transaction.chain_id())
