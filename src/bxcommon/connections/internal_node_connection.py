...
class InternalNodeConnection(AbstractConnection):
    ...
    def __init__(self, sock, address, node, from_me=False):
        ...
        self.protocol_version = versioning.CURRENT_PROTOCOL_VERSION
    ...
    def set_protocol_version_and_message_factory(self):
        ...
        protocol_version = versioning.get_connection_protocol_version(self.inputbuf)

        if protocol_version is None:
            return False

        if not versioning.is_protocol_supported(protocol_version):
        ...
        self.message_factory = versioning.get_message_factory_for_version(protocol_version)
    ...
    def enqueue_msg(self, msg, prepend=False):
        ...
        if self.protocol_version < CURRENT_PROTOCOL_VERSION:
            versioned_message = versioning.convert_message_to_older_version(self.protocol_version, msg)
        else:
    ...
    def pop_next_message(self, payload_len):
        ...
        if msg is None or self.protocol_version >= CURRENT_PROTOCOL_VERSION:
            return msg

        versioned_msg = versioning.convert_message_from_older_version(self.protocol_version, msg)
