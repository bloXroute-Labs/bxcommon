...
class BroadcastMessageV5(AbstractBloxrouteMessage):
    ...
    def __init__(self, msg_hash=None, network_num=None, is_encrypted=False, blob=None, buf=None):
        if buf is None:
            msg_buf = bytearray(
                self.HEADER_LENGTH + SHA256_HASH_LEN + constants.NETWORK_NUM_LEN + constants.BLOCK_ENCRYPTED_FLAG_LEN +
            ...
            super(BroadcastMessageV5, self).__init__(self.MESSAGE_TYPE, off - self.HEADER_LENGTH, msg_buf)
        else:
