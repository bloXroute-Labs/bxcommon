...
class TxMessage(AbstractBroadcastMessage):
    PAYLOAD_LENGTH = AbstractBroadcastMessage.PAYLOAD_LENGTH + _SID_LEN
    ...
    def __init__(self, message_hash: Optional[Sha256Hash] = None, network_num: Optional[int] = None,
                 source_id: str = "", short_id: int = _NULL_SID,
        ...
        if tx_val is not None:
            self.PAYLOAD_LENGTH = AbstractBroadcastMessage.PAYLOAD_LENGTH + _SID_LEN + len(tx_val)
    ...
    def tx_val(self) -> memoryview:
        if self._tx_val is None:
            if self.payload_len() == 0:
                ...
                off = self.HEADER_LENGTH + AbstractBroadcastMessage.PAYLOAD_LENGTH + _SID_LEN - \
