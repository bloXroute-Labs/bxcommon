...
protocol_to_bdn_tx_to_bx_tx: Dict[BlockchainProtocol, Callable[
    [Union[bytes, bytearray, memoryview], int, Optional[TransactionFlag]],
    TxMessage
]]
...
def bdn_tx_to_bx_tx(
    ...
    return protocol_to_bdn_tx_to_bx_tx[blockchain_protocol](raw_tx, network_num, transaction_flag)
...
def ont_bdn_tx_to_bx_tx(
    ...
    try:
        ...
        raise ValueError("Invalid raw transaction provided.")
    return TxMessage(
        message_hash=tx_hash, network_num=network_num, tx_val=raw_tx, transaction_flag=transaction_flag
    )
